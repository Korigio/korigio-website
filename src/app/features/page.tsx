...
export default async function FeaturesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-16 sm:py-20">
      ...
      <p className="mt-5 max-w-2xl text-lg text-zinc-400">{dict.features.subtitle}</p>
      <div className="mt-12 grid gap-4 md:grid-cols-2">
        {dict.features.items.map((item) => (
          <article
            key={item.title}
            className="rounded-[28px] border border-white/8 bg-white/3 p-7"
          >
      ...
      <div className="mt-16">
        ...
        <ol className="mt-8 grid gap-4 md:grid-cols-4">
          {dict.workflow.steps.map((step) => (
            <li key={step.n} className="rounded-3xl border border-white/8 p-5">
